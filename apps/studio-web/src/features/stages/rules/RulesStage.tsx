...
export function RulesStage({
  ...
  const removeRuleParameter = (parameterId: string) => {
    const parameter = draft.parameterDefinitions.find((item) => item.id === parameterId);
    if (!parameter) return;
    const escapedId = parameterId.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const token = new RegExp(`\\b${escapedId}\\b`);
    const expressionKey = (key: string) => key === "expression" || key.endsWith("Expression");
    const metadataKey = (key: string) =>
      key === "id" || key === "label" || key === "displayName" || key === "description" || key === "aliases";
    const hasDirectReference = (value: unknown, key = ""): boolean => {
      if (typeof value === "string") return !expressionKey(key) && !metadataKey(key) && value === parameterId;
      if (Array.isArray(value)) return value.some((item) => hasDirectReference(item, key));
      if (value && typeof value === "object") {
        return Object.entries(value).some(([childKey, childValue]) =>
          childKey !== "parameterDefinitions" && hasDirectReference(childValue, childKey),
        );
      }
      return false;
    };
    const directReference = hasDirectReference(draft);
    if (directReference) {
      setRuleParameterError(`参数 ${parameterId} 存在直接绑定，请先解除绑定后再删除。`);
      return;
    }
    if (!window.confirm(`确定删除预声明参数“${parameterId}”吗？删除后该参数将从契约页一并移除。`)) return;
    const literal = String(parameter.default);
    const replaceExpressions = (value: unknown, key = ""): unknown => {
      if (typeof value === "string") {
        return key === "expression" || key.endsWith("Expression") ? value.replace(token, literal) : value;
      }
      if (Array.isArray(value)) return value.map((item) => replaceExpressions(item, key));
      if (value && typeof value === "object") {
        return Object.fromEntries(Object.entries(value).map(([childKey, childValue]) => [childKey, replaceExpressions(childValue, childKey)]));
      }
      return value;
    };
    const rewritten = replaceExpressions(draft) as Draft;
    change({
      ...rewritten,
      parameterDefinitions: rewritten.parameterDefinitions.filter((item) => item.id !== parameterId),
    });
    setRuleParameterError("");
  };
  ...
  return (
    <>
      ...
      <RuleParameterPanel
        ...
        removeRuleParameter={removeRuleParameter}
      />

...
def execute(client: RuiWareApiClient, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    result = client.post("/template-drafts/create", {"name": name, "confirmed": confirmed})
    draft = result.get("draft", result)
    draft_id = draft.get("id") if isinstance(draft, dict) else None
    workspace = client.put("/workspace/current-draft", {"draftId": draft_id, "confirmed": confirmed}) if draft_id else None
